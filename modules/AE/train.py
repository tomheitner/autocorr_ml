...
class ConvolutionalAutoencoderTrainer:
    ...
    def train(
        ...
        for epoch in epoch_iter:            
            # save checkpoint
            ckpt_save_path = f'./ae_model_checkpoints/ae_model_{str(datetime.now().date())}_ckpt_{int(epoch*train_samples)}.pt'
            torch.save(self.network.state_dict(), ckpt_save_path)
            
            # save checkpoint's logs
            log_ckpt_save_path = f'./ae_model_checkpoints/ae_model_{str(datetime.now().date())}_log_{int(epoch*train_samples)}.pkl'
            ...
            for sample_idx in val_iter:
                peak, _ = next(validation_set)           
                with torch.no_grad():
                    #  sending validation peak to self.device
                    peak = peak.float().to(self.device)
                    #  reconstructing peak
                    output = self.network(peak)
                    #  computing validation loss
                    # print(f"out shape {output.shape}, in shape {peak.shape}")
                    val_loss = loss_patch(output, peak, loss_function)
